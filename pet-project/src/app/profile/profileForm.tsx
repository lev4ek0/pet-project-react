...
const EmailLifetime = parseInt(process.env.NEXT_PUBLIC_EMAIL_LIFETIME || "") || 60 * 60 * 24

export function ProfileForm({ isLoading, name, email, avatar }: ProfileFormProps) {
    ...
    const newEmailWarning = <Alert variant="warn">
    ...
    const inputAvatar = isLoading ? <Skeleton className="h-20 w-20 rounded-full" /> :
        <Avatar className="w-20 h-20">
            <AvatarImage src={avatar} />
            <AvatarFallback><Skeleton className="h-20 w-20 rounded-full" /></AvatarFallback>
        </Avatar>

    const inputUsername = isLoading ? <Skeleton className="h-9 w-full" /> : <Input
        id="name"
        value={nameForm}
        onChange={(e) => setName(e.target.value)}
    />

    const inputEmail = isLoading ? <Skeleton className="h-9 w-full" /> : <Input
        id="email"
        type="email"
        value={emailForm}
    ...
    useEffect(() => {
        if (newEmail && (newEmail === email || (updatedAt || 0) + EmailLifetime < Math.floor(new Date().getTime() / 1000))) {
        ...
        setEmail(email || "")
        setName(name || "")
    }, [newEmail, email, updatedAt, setNewEmail]);
